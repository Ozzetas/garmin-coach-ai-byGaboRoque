...
# Logging estructurado: Prohibido usar prints genéricos en producción
...
class Activity(BaseModel):
    ...
    avg_heart_rate: int | None = Field(default=None, description="BPM promedio")

    @field_validator("distance_meters")
    @classmethod
    def validate_gps_distance(cls, value: float) -> float:
        if value < 0.0:
            logger.error(f"Integridad comprometida: Distancia negativa ({value}m).")
            raise ValueError("La distancia no puede ser negativa.")
        return value

    @field_validator("avg_heart_rate")
    @classmethod
    def validate_biological_limits(cls, value: int | None) -> int | None:
        if value is not None and (value < 30 or value > 220):
            logger.error(f"Lectura anómala del sensor: {value} BPM.")
